...
public class TextureSimulator : MonoBehaviour {
    ...
    public RenderTexture velocityTexture0 => _velocitySrc;
    ...
    public void RebuildTrailTexture() {
        ...
        var ramp = new Texture2D(TAIL_RAMP_RESOLUTION, 1, TextureFormat.Alpha8, mipChain: false, linear: true);
    ...
    private void Update() {
        ...
        if (_manager.simulationEnabled) 
        {
            ...
            if (_dynamicTimestepEnabled)
            {
                ...
                var lerpValue = Mathf.InverseLerp(_currSimulationTime, _prevSimulationTime, _currScaledTime);
    ...
    private IEnumerator restartCoroutine(EcosystemDescription ecosystemDescription, ResetBehavior resetBehavior) 
    {
        ...
        var layout = new List<SpeciesRect>();

        var isUsingOptimizedLayout = tryCalculateOptimizedLayout(ecosystemDescription.toSpawn, layout);
