Shader "Unlit/Simulation"
{
    ...
    #define tex2Dlod0(samp, s) tex2Dlod(samp, half4(s, 0,0))
    ...
    struct Interpolator_p
    {
        float4 uv : TEXCOORD0;
        float4 vertex : SV_Position;
    };

    struct Interpolator_i
    {
        float4 uv : TEXCOORD0;
        float4 social : TEXCOORD1;
    ...
    Interpolator_p vert_p(meshData_p v)
    {
        Interpolator_p o;
    ...
    Interpolator_i vert_i(meshData_i v)
    {
        Interpolator_i o;
        o.vertex = UnityObjectToClipPos(v.vertex);
        o.uv = v.uv;
    ...
    half4 integratePositions(Interpolator_p i) : SV_Target
    {
        ...
        //Dont hit the head and Hands pls
    ...
    half4 globalForces(Interpolator_p i) : SV_Target
    {
    ...
    half4 dampVelocities(Interpolator_p i) : SV_Target
    {
        half4 velocity = tex2Dlod0(_ParticleVelocities, i.uv.xy);
        half4 particle = tex2Dlod0(_ParticlePositions, i.uv.xy);
    ...
    FragmentOutput updateCollisionVelocities(Interpolator_i i)
    {
        half4 particle = tex2Dlod0(_ParticlePositions, i.uv.xy);
    ...
    for (half y = i.sampleRect.y; y < i.sampleRect.w; y += (1.0 / 64.0)) {
      for (half x = i.sampleRect.x; x < i.sampleRect.z; x += (1.0 / 64.0)) {
        ...
        // Velocity of Particles and Social Behaviour
    ...
    half4 stepSocialQueue(Interpolator_p i) : SV_Target
    {
        // Gravitation Towards a Coordinate
    ...
    float4 debugOutput(Interpolator_p i) : SV_Target
    {
    ...
    half4 copy(Interpolator_p i) : SV_Target
    {
        return tex2Dlod0(_CopySource, i.uv.xy);
    }

    half4 randomizeInit(Interpolator_p i) : SV_Target
    {
    ...
    SubShader
    {
        ...
        //Pass 0: integrate velocities and Positions
        Pass
        {
            CGPROGRAM
        ...
        //Pass 2: global forces and Gravity
        Pass
        {
            CGPROGRAM
        ...
        //Pass 3: damp velocity and add offset social forces(Main Social Behaviour Algorithm)
        Pass
        {
            CGPROGRAM
            #pragma vertex vert_p
